...
local parser = require"CFLOW.ParseLua"
local ParseLua = parser.ParseLua
local util = require'CFLOW.Util'
...
local function Format_Beautify(ast)
	...
	formatStatement = function(statement)
		...
		elseif statement.AstType == 'WhileStatement' then
			-- for i,v in pairs(statement) do print(i,v) end error''
			...
			if statement.CREATE_STATE then
				if statement.CREATE_STATE.locals and statement.CREATE_STATE.locals.AstType then 
					out = joinStatementsSafe(getIndentation() .. formatStatement(statement.CREATE_STATE.locals) .. EOL, out)
				else 
					-- error 'f'
				end
				out = joinStatementsSafe(getIndentation() .. "local " .. statement.CREATE_STATE.name .. " = " .. statement.CREATE_STATE.value .. EOL, out)
			end
			-- indent = indent - 1
			out = joinStatementsSafe(out, getIndentation() .. "end") .. EOL
			-- print(out)
